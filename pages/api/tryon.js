...
function streamToString(stream) {
  ...
  if (typeof stream.on !== "function") {
    ...
    if (stream instanceof ArrayBuffer) {
      return Promise.resolve(Buffer.from(new Uint8Array(stream)).toString("utf8"));
    }
...
export default async function tryonHandler(req, res) {
  ...
  try {
    ...
    console.log("Person image:", personImgFile.originalFilename, "size:", personImgFile.size);
    console.log("Garment image:", garmentImgFile.originalFilename, "size:", garmentImgFile.size);
    ...
    await Promise.all([validateImageUrl(humanImageUrl), validateImageUrl(clothImageUrl)]);
    ...
    if (!postResponse.ok) {
      ...
      throw new Error(`Kling API error: ${postResponse.status} ${postResponse.statusText}`);
    }
    ...
    while (Date.now() - startWaitTime < MAX_WAIT_TIME_MS) {
      console.log(`Polling task status... (${Math.round((Date.now() - startWaitTime) / 1000)}s elapsed)`);
      ...
      if (!queryResponse.ok) {
        throw new Error(`Kling API request failed: ${queryResponse.status} ${queryResponse.statusText}`);
      }
      ...
      if (status === "succeed") {
        ...
        if (resultImageUrl) {
          try {
            ...
            if (!finalResp.ok) {
              throw new Error(`Failed to fetch generated image: ${finalResp.statusText}`);
            }
            const finalContentType = finalResp.headers.get("content-type") || "image/jpeg";
            let extension = "jpeg";
            ...
            const finalUrl = await uploadToS3AndGetUrl(imageBuffer, generatedImageName, "generated-image");
